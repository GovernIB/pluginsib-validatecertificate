 ...
 * 
 */
public class AfirmaCxfCertificatePlugin extends AbstractPluginProperties implements ICertificatePlugin {
    ...
    public static final String AUTH_KS_CERT_PASSWORD = BASE_PROPERTIES + "authorization.ks.cert.password";
    ...
    private ResultatValidacio checkValidaCertificat(X509Certificate cert, boolean obtenirDadesCertificat)
            throws Exception {
        ...
        if (validaCertificat == null) {
            ...
            if (getProperty(AUTH_UP_USERNAME) != null) {
                ...
                validaCertificat = new ValidaCertificat(endPoint, applicationID, modeValidacio, username, password,
                        debug);
                ...
                validaCertificat = new ValidaCertificat(endPoint, applicationID, modeValidacio, path, type, password,
                        alias, certPassword, debug);
    ...
    private void checkNullProperty(String key, String value) throws Exception {
        if (value == null) {
            throw new Exception("Property " + key + " for " + this.getClass().getName() + " must be defined.");
        }
    }
