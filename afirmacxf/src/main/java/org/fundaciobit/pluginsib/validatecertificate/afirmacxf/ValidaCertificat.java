...
import org.fundaciobit.pluginsib.validatecertificate.afirmacxf.validarcertificadoapi.ValidacionCadenaInfo;
import org.fundaciobit.pluginsib.validatecertificate.afirmacxf.validarcertificadoapi.ValidacionEstadoInfo;
import org.fundaciobit.pluginsib.validatecertificate.afirmacxf.validarcertificadoapi.ValidacionService;
import org.fundaciobit.pluginsib.validatecertificate.afirmacxf.validarcertificadoapi.ValidacionSimpleInfo;
...
public class ValidaCertificat {
    ...
    public static final int MODE_VALIDACIO_AMB_REVOCACIO_I_AMB_CADENA = 3;
    ...
    private ValidaCertificat(String endPoint, String aplicacioId, int modeValidacio, ClientHandler clientHandler,
            boolean debug) {
    ...
    public ValidaCertificat(String endPoint, String application_id, int modeValidacio, String username, String password,
            boolean debug) {
        this(endPoint, application_id, modeValidacio, new ClientHandlerUsernamePassword(username, password), debug);
    }

    public ValidaCertificat(String endPoint, String application_id, int modeValidacio, String keystoreLocation,
            String keystoreType, String keystorePassword, String keystoreCertAlias, String keystoreCertPassword,
            boolean debug) {

        this(endPoint, application_id, modeValidacio, new ClientHandlerCertificate(keystoreLocation, keystoreType,
                keystorePassword, keystoreCertAlias, keystoreCertPassword), debug);
    }
    ...
    public ResultatValidacio validar(X509Certificate certificate, boolean obtenirDadesCertificat) throws Exception {
    ...
    public ResultatValidacio validar(byte[] data, boolean obtenirDadesCertificat) throws Exception {
        ...
        if (ex == null) {
            ...
            StringBuffer detallsError = new StringBuffer();

            {
                ValidacionSimpleInfo vs = rvi.getValidacionSimple();
                if (vs != null
                        && !String.valueOf(ResultatValidacio.RESULTAT_VALIDACIO_OK).equals(vs.getCodigoResultado())
                        && vs.getDescResultado() != null) {
                    if (detallsError.length() != 0) {
                        detallsError.append(", ");
                    }
                    detallsError.append(vs.getDescResultado());
                }
            }
            {
                ValidacionEstadoInfo vs = rvi.getValidacionEstado();
                if (vs != null && !String.valueOf(ResultatValidacio.RESULTAT_VALIDACIO_OK).equals(vs.getEstado())
                        && vs.getDescEstado() != null) {
                    if (detallsError.length() != 0) {
                        detallsError.append(", ");
                    }
                    detallsError.append(vs.getDescEstado());
                }
            }
            {
                ValidacionCadenaInfo vs = rvi.getValidacionCadena();
                if (vs != null
                        && !String.valueOf(ResultatValidacio.RESULTAT_VALIDACIO_OK).equals(vs.getCodigoResultado())
                        && vs.getDescResultado() != null) {
                    if (detallsError.length() != 0) {
            ...
            if (detallsError.length() == 0) {
                ...
                resultatValidacio
                        .setResultatValidacioDescripcio(rvi.getDescripcion() + "(" + detallsError.toString() + ")");
            }
    ...
    private String cridarValidarCertificado(String certificatBase64, boolean obtenirDadesCertificat, int modeValidacio)
            throws Exception {
        ...
        String xmlPeticio = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>"
                ...
                + "<peticion>ValidarCertificado</peticion>" + "<versionMsg>1.0</versionMsg>" + "<parametros>"
                + "<certificado><![CDATA[" + certificatBase64 + "]]></certificado>" + "<idAplicacion>" + aplicacioId
                + "</idAplicacion>" + "<modoValidacion>" + modeValidacio + "</modoValidacion>" + "<obtenerInfo>"
                + obtenirDadesCertificat + "</obtenerInfo>" + "</parametros>" + "</mensajeEntrada>";
